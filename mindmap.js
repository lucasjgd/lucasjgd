...
function selectNode(e){
	...
	document.forms["skill"].elements[elem.name].readOnly = true;
	bindedFields.forEach((elem) => {// Cette ligne pacours le tableau bindedFields (defini ligne 2)
		document.forms["skill"].elements[elem.name].value = currentSkill.hasOwnProperty(elem.name) ? currentSkill[elem.name] : "";	
		// Cette ligne affiche donc chaque elemant parcouru dans le tableau
	});
...
addEventListener("DOMContentLoaded", (ev) => {
    fetch("Veille-Technologique.json")
